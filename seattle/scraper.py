...
from utils import CacheRetriever, SocrataRetriever
...
#        self.created_newsitem_ids.append(ni.id)
    ...
    def get_html_and_headers(self, uri, data=None, headers=None, send_cookies=True, follow_redirects=True, raise_on_error=True, basic_auth=None):
        ...
        if basic_auth is not None:
            import base64
            auth_header = 'Basic %s' % base64.encodestring('%s:%s' % (basic_auth[0], basic_auth[1])).strip()
...
class EventScraper(Scraper):
    "Use this subclass for creating new event scrapers."
    schema = 'events'
    primary_key = ('unique_id',)

    # Full name of the event data source.
    source = None

    # Do not add an event if it has the same title, item_date, location, source,
    # and time as another event.
    check_for_duplicates = False

    # If a future event is not encountered in the latest scrape, mark it as
    # cancelled.
    mark_unseen_items_as_cancelled = False

    def run(self, *args, **kwargs):
        if self.mark_unseen_items_as_cancelled:
            self.num_cancelled = 0
            self.seen_newsitem_ids = set()

        results = super(EventScraper, self).run(*args, **kwargs)

        if self.mark_unseen_items_as_cancelled:
            source_field = SchemaField.objects.get(schema__slug='events',
                name='source')

            # Check all future events from this source. If it wasn't seen during
            # the current scrape, then assume that it was cancelled and should
            # therefore be marked as such.
            tomorrow = self.start_date + datetime.timedelta(days=1)
            qs = NewsItem.objects.filter(schema__slug='events',
                item_date__gte=tomorrow, is_public=True)
            qs = qs.by_attribute(source_field, self.source, is_lookup=True)
            for ni in qs:
                if ni.attributes['unique_id'] not in self.seen_newsitem_ids:
                    ni.attributes['is_cancelled'] = True
                    self.num_cancelled += 1
                    mesg = ('Marked newsitem %s as cancelled because it was '
                        'not seen.' % ni.id)
                    self.logger.info(mesg)

            self.logger.info('Records marked as cancelled: {0}'.format(
                self.num_cancelled))

        return results

    def prepare_data(self, datadict):
        "Automatically set or adjust common attributes for event newsitems."
        datadict['pub_date'] = self.get_pub_date(datadict['item_date'])
        datadict['category'] = datadict.get('category', '')
        if not datadict.get('source'):
            datadict['source'] = self.source
        if not datadict.get('organizer'):
            datadict['organizer'] = None
        datadict['source__text_slug'] = True
        if not 'is_cancelled' in datadict:
            datadict['is_cancelled'] = False

        return super(EventScraper, self).prepare_data(datadict)

    def save(self, datadict):
        """
        Override save() so that we can ignore events with dates that are too
        old. And do some other stuff, depending on which class attributes are
        True.
        """
        today = datetime.date.today()
        if datadict['item_date'] < today:
            return
        if self.check_for_duplicates and self.event_already_exists(datadict):
            return
        if self.mark_unseen_items_as_cancelled:
            self.seen_newsitem_ids.add(datadict['unique_id'])

        super(EventScraper, self).save(datadict)

    def event_already_exists(self, datadict):
        """
        Returns True if there already exists an event newsitem with the same
        title, item_date, location, source, and time, but NOT the same
        unique_id.
        """
        schema = Schema.objects.get(slug='events')
        time_field = schema.schemafield_set.get(name='time')
        source_field = schema.schemafield_set.get(name='source')
        unique_id_field = schema.schemafield_set.get(name='unique_id')

        qs = NewsItem.objects.filter(schema=schema, title=datadict['title'],
            item_date=datadict['item_date'], location=datadict['location'])
        qs = qs.by_attribute(time_field, datadict['time'])
        qs = qs.by_attribute(source_field, datadict['source'], is_lookup=True)
        qs = qs.extra(where=[
            "db_attribute.{0} <> '{1}'".format(unique_id_field.real_name, datadict['unique_id']),
        ])
        return qs.exists()

    def get_pub_date(self, date, threshold_days=14):
        """
        If the given datetime.date is more than threshold_days into the future,
        this returns the date minus threshold_days, otherwise it returns the
        current date.
        Note the return value is a datetime.datetime, not a datetime.date. This
        is so that the return value can be used as a NewsItem.pub_date.
        """
        threshold = datetime.timedelta(days=threshold_days)
        if date > self.start_date + threshold:
            date = date - threshold
        else:
