...
from retrieval_base import ScraperBroken
from newsitem_list_detail import NewsItemListDetailScraper
...
detail_url = lambda list_record: 'http://houston-tx.healthinspections.us/media/search.cfm?q=d&f=%s&i=%s' % (list_record['business_id'], list_record['inspection_id'])
...
class RestaurantScraper(NewsItemListDetailScraper):
    ...
    parse_list_re = re.compile(r'(?si)'
        '<tr[^>]*>\s*'
        '<td[^>]*>\s*<a href="[^"]+&f=(?P<business_id>[^&]+)[^"]*&i=(?P<inspection_id>[^&"]+)[^"]*">(?P<business_name>.*?)</a>\s*<br>\s*'
        '(?P<address>[^<]*?)\s*</td>\s*'
        '<td[^>]*>\s*(?P<site>[^<]*?)\s*</td>\s*'
        '<td[^>]*>\s*(?P<inspection_date>\d\d?/\d\d?/\d\d\d\d)\s*</td>\s*'
        '<td[^>]*>\s*(?P<status>[^<]*?)\s*</td>'
    )
    parse_detail_re = re.compile(r'(?si)'
        '<td[^>]*>Insp\. Date</td>\s*'
        '<td[^>]*>Insp\. Site</td>\s*'
        '<td[^>]*>Status</td>\s*'
        '<td[^>]*>Activity</td>\s*</tr>\s*</table>\s*'
        '<table[^>]*>\s*<tr[^>]*>\s*<td[^>]*>(?P<inspection_date>\d\d?/\d\d?/\d\d\d\d)</td>\s*'
        '<td[^>]*>(?P<site>[^<]*)</td>\s*'
        '<td[^>]*>(?P<status>[^<]*)</td>\s*\s*'
        '<td[^>]*>(?P<activity>[^<]*)</td>\s*</tr>.*?'
        '<table[^>]*>(?P<violations_html>.*?)</table>'
    )
    sleep = 2
    ...
    def list_pages(self):
        ...
        for code, facility_type in FACILITY_TYPES:
            ...
            html = self.get_html('http://houston-tx.healthinspections.us/media/search.cfm', data, send_cookies=False)
...
if __name__ == "__main__":
    ...
    end_date = datetime.date(2010, 9, 29)
