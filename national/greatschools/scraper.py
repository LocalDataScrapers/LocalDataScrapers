...
class Scraper(object):
	...
	filename = 'schoolreviews-data.txt'
	ni = []
        try:
            got_error = True
            for datadict in self.data():
...
#            from django.db import connection
 #           connection._rollback()
    ...
    def prepare_data(self, datadict):
        ...
        # Calculate location.
        datadict['location_name'] = datadict.get('location_name', None)
	datadict['location_name_geocoder'] = datadict.get('location_name_geocoder', None)
	datadict['review_id'] = datadict.get('review_id', None)
	datadict['school_name'] = datadict.get('school_name', None)
	datadict['stars'] = datadict.get('stars', None)
	datadict['submitter'] = datadict.get('submitter', None)
	datadict['source'] = datadict.get('source', None)
        # Calculate title.
        TITLE_MAXLENGTH = 255
...
#        self.created_newsitem_ids.append(ni.id)
	...
	ni['location_name_geocoder'] = datadict['location_name_geocoder']
	ni['review_id'] = datadict['review_id']
	ni['school_name'] = datadict['school_name']
	ni['stars'] = datadict['stars']
	ni['submitter'] = datadict['submitter']
	ni['source'] = datadict['source']
